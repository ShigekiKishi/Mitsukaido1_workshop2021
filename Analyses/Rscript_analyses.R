...
# それぞれに保存したファイルをバイナリとして読み込む

#write.table(sdat3[1], file="sdat3.txt")

rdat3 <- file("./Analyses/sdat3.txt", "rb")
...
pw <- ggplot(data = waved, aes(x=time, y=wave))+
  ...
  geom_line()+
  ylab("Wave length")+xlab(">> Time >>")+ylim(c(0,256))
pw
...
p <- ggplot(data=freq2, aes(x=name, y=sound, fill=value %>% sqrt()))+
  ...
  scale_fill_gradientn("value", colours = colormap("jet"))+ # "rainbow"
